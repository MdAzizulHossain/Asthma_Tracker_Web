...
class _SHYWord(list):
    ...
    def shyphenate(self, newWidth, maxWidth):
        ...
        R = [ww - L[0] + shyLen] + (
            [] if not right else [(f, _SHYIndexedStr(right, [_ - x for _ in X[j + 1:]]))]) + self[i1 + 1:]
    ...
    def breakLines(self, width):
        ...
        if (nFrags == 1
                ...
                if newWidth > maxWidth + spaceShrink and not (isinstance(word, _SplitWordH) or forcedSplit):
                    ...
                    if splitLongWords and not (isinstance(word, _SplitWord) or forcedSplit):
                        ...
                        if wordWidth > max(maxWidths[nmw:nmw + 1]):
                            # a long word
                            words[0:0] = _splitWord(word, currentWidth + spaceWidth, maxWidths, lineno, fontName,
                                                    fontSize, self.encoding)
            ...
            while _words:
                ...
                if not lineBreak and newWidth > (maxWidth + spaceShrink) and not isinstance(w,
                                                                                            _SplitFragH) and not hasattr(
                        f, 'cbDefn'):
